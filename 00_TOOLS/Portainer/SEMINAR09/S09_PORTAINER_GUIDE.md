...
> *▸ "Up to now you have worked with a single Python script. From today onward we have infrastructure — multiple services running at the same time on an isolated network. To see everything at a glance, we open Portainer in the browser."*
...
4. Click `seminar9_client1` → **Console** → **Connect**. A shell opens in the browser.
   > *▸ "This is the equivalent of `docker exec -it seminar9_client1 sh` — but without having to type the command."*

5. Click **Networks** in the sidebar → click `ftpnet`. All three containers are listed with their IPs.
   > *▸ "The containers are on the same bridge network. The server has one IP address, the clients have others. Exactly like a physical LAN — except that it is virtual."*

6. Transition back to the normal seminar flow:
   > *▸ "Keep Portainer open in the browser. Use it at any time for logs, terminals, or if you are no longer sure what containers are running. It does not replace the CLI — it is an additional tool."*
